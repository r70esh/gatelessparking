    ...
    return (
        <div className='-mt-16 p-4'>
            <div className="sm:container bg-gray-50 shadow">
                <header className="text-2xl sm:text-4xl text-center w-full p-4">
                    My Bookings
                </header>
                <hr />

                {bookings.map(booking => {
                    const location = booking.locationid as ParkingLocation | null
                    ...
                    return (
                        <div key={booking.id}>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 p-2 items-center">
                                ...
                                {booking.status === BookingStatus.BOOKED && location && (
                                    <div className='flex sm:flex-col sm:space-y-2 items-end'>
                                        <CancelBookingButton
                                            param={JSON.parse(JSON.stringify(booking.id))}
                                        />
                                        <EditBookingButton
